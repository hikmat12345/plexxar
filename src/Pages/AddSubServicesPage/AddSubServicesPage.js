...
import { getSubServices, addServices } from "./actions";
import { objectIsEmpty } from "../../utils";
import history from "../../history";

//scss
import "./AddSubServicesPage.scss";
...
const AddSubServicesPage = () => {
  ...
  useEffect(() => {
    if (userCountryId) {
      getSubServicesAPI(serviceId);
    }
  }, [serviceId, userCountryId]);
  ...
  const handleAddService = (serviceIdd) => {
    let selectedPref = selectPref.find((a) => a.id === serviceIdd).selected;
        ...
        <div className="add-services-container">
          {servicesList.map((service) => (
            <Accordion
              ...
              onClick={() =>
                service.hasSubservice
                  ? getSubServicesAPI(service.serviceId)
                  : service.hasChilds
...
export default PlexaarContainer(AddSubServicesPage);
