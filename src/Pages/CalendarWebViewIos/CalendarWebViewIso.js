...
//scss
import "./CalendarWebViewIso.scss";

const CalendarWebViewIso = () => {
  ...
  useEffect(() => {
    ...
    //         description: "break...",
    //         start:
    //         ob.unAvailabilityDate.split("T")[0] + "T" + ob.timeFrom + ":00",
    //         end: ob.unAvailabilityDate.split("T")[0] + "T" + ob.timeTo + ":00",
    //         color: "#cdcdcd",
    //         isBreak: 1,
    //       })
    //     );
  ...
  function setSlotInfo(resourceId, title, datetime) {
    ...
    getAvailableDuration({
      ...
      startStr,
      callback: (res) =>
        window.webkit.messageHandlers.setSlotInfo.postMessage({
          availableDur: parseInt(res),
          provider: parseInt(resourceId),
          isActive: resources.find((a) => a.id === parseInt(resourceId)).isActive,
          providerName: title,
          slot: startStr,
        }),
    });
  ...
  function setAppointmentInfo(id, resourceId, isBreak, sessionId) {
    window.webkit.messageHandlers.setAppointmentInfo.postMessage({
      id: parseInt(id),
      isActive: resources.find((a) => a.id === parseInt(resourceId)).isActive,
      isBreak: isBreak,
      sessionId: sessionId,
...
export default CalendarWebViewIso;
